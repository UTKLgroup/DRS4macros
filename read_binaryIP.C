...
void decode(char *filename) {
 ...
 for(chn = 0; chn < (datfh.inpch).size(); chn++){
   ...
   cout << "1st comment: channel number is " << chn << endl;
   ...
   cout << "2nd comment : channel number is " << chn << endl; 
   ...
   for (n=0; n < quer.ev; n++){
     ...
     cout << "3rd comment: channel number is " << chn << endl;
	 ...
	 gPad->WaitPrimitive();
       ...
       FormAreaVec[chn].Fill(WaveformArea);
   ...
   //hAllWaveforms[chn]->Write();
     cout << "Why oh god why!?!?!!?!?? " << endl;
     FormAreaVec[chn].Write();
   /*TailArea->Write();
     TAvsWA->Write();
     TAoverWA->Write();
     HvsTAoverWA->Write();
