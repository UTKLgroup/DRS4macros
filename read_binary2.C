...
void decode(char *filename) {
  ...
  TH2D *hAllWaveforms = new TH2D("hAllWaveforms","hAllWaveforms",1024,0,600,100,-0.1,0.05);
  ...
  TH2F *PHvPA = new TH2F("PHvPA","Pulse height vs Pulse Area",1000,-8,2,600,-0.2,0.1);
  TH1F *HR = new TH1F("HR","Ratio of Pulse height over pulse area",100,-0.2,0.2);
  TH2F *TAvToA = new TH2F("TAvTOA","Tail Area vs Total Area",200,-2,1,2000,-10,10);
  // read time header
  ...
  double tailintegral=0.0;
    for(int i = 1 ; i < 900 ; i++) {
      gIntegralVBW2=gIntegralVBW2+(waveform[0][i])*((time[0][i]-time[0][i-1])/2.0+(time[0][i+1]-time[0][i])/2.0);
             }
  //Tail integral 
    for(int i=TailStarts ; i < 900 ; i++) {
      tailintegral=tailintegral+(waveform[0][i])*((time[0][i]-time[0][i-1])/2+(time[0][i+1]-time[0][i])/2.0);
	}
  ...
  double RatioHR = temp/gIntegralVBW2;
  // Fill Histogram
  h1->Fill(ChargeCount);
  TT->Fill(ChargeRatio);
  Tail->Fill(TailCharge);
  TAvToA->Fill(ChargeCount,TailCharge);
